...
public class UserController {
    ...
    public ServerResponse<User> login(String username, String password, HttpSession session){
        ...
        if(response.isSuccess()){
            session.setAttribute(Const.CERRENT_USER,response.getData());
        }
        return response;
    ...
    public ServerResponse<String> logout(HttpSession session){
        session.removeAttribute(Const.CERRENT_USER);
    ...
    public ServerResponse<User> updateInformation(HttpSession session,User user){
        User currentUser = (User)session.getAttribute(Const.CERRENT_USER);
        if(null == currentUser){
            return ServerResponse.createByErrorByMessage("用户未登录");
        }
